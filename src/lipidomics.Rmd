...
for (lipid in lipids_in_common) {
  ...
  p = cor.test(combined$totalLipid.x, combined$totalLipid.y, 
                  method=c("pearson", "kendall", "spearman"))$p.value
  
  df = rbind(df, data.frame("LipidClass" = lipid, "Spearman_corr" = corr, "p_val" = p))
}

# save df
write.csv(df, '../processed_data/misc/liver_plasma_lipid_class_corr.csv', row.names=FALSE)
