...
# By 95% confidence ellipse, Mice 53 and 39 are outliers. We will remove them and re-plot the PCA. They are also excluded from the remaining analysis.
```
...
colors <- c("#A00000", "#00C000", "#5757F9", "#FF6000", "#0000C0", "#C0C000", "#D7B0B0", "#9F044D", 
            "#077E97", "#C5944E", "#034E61", "#FFA040", "#606060", "#af05d3")
...
pval_table %>%
  ...
  ggplot(aes(x=log2FC, y=log10Abundance, size=log10P, fill=RefinedClass)) +
    ...
    geom_hline(yintercept = c(-2, 0, 2, 4), linetype = 'dotted', color = 'grey70', size = 1) +
...
df = data.frame("LipidClass" = character(), "Spearman_corr" = numeric())
for (lipid in lipids_in_common) {
  lipid_in_liver = totalLipidbyClass_liver %>% filter(RefinedClass == lipid)
  lipid_in_plasma = totalLipidbyClass_plasma %>% filter(RefinedClass == lipid)
   
  combined = left_join(lipid_in_liver, lipid_in_plasma, by = 'ID')
  corr = cor.test(combined$totalLipid.x, combined$totalLipid.y, 
                  method=c("pearson", "kendall", "spearman"))$estimate
  
  df = rbind(df, data.frame("LipidClass" = lipid, "Spearman_corr" = corr))
}

df %>% ggplot(aes(x = LipidClass, y = Spearman_corr)) + 
  geom_bar(stat = 'identity', fill = 'gray70', color = 'black', size=0.5) +
  geom_hline(yintercept=1, color='maroon', linetype='dashed') +
  geom_hline(yintercept=-1, color='maroon', linetype='dashed') +
  labs(y = 'Spearman correlation', x = 'Total lipid by class')

ggsave('../figures/lipidomics_plasma_liver_total_corr.pdf', height = 5, width = 10)
```
