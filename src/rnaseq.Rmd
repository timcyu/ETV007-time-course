...
genes02 = as.data.frame(res02) %>% tibble::rownames_to_column(var = "Ensembl.Gene.ID") %>% left_join(., mapping, by = "Ensembl.Gene.ID") %>% mutate(DE_02 = ifelse(padj < 0.05 & abs(log2FoldChange) > log2(1.5), "yes", "no")) %>% dplyr::select(Associated.Gene.Name, DE_02)

res28 = lfcShrink(dds, contrast = c("condition", "8","2"), type = 'ashr')
genes28 = as.data.frame(res28) %>% tibble::rownames_to_column(var = "Ensembl.Gene.ID") %>% left_join(., mapping, by = "Ensembl.Gene.ID") %>% mutate(DE_28 = ifelse(padj < 0.05 & abs(log2FoldChange) > log2(1.5), "yes", "no")) %>% dplyr::select(Associated.Gene.Name, DE_28)
