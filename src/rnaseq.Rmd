...
### vertical lines indicate |FC| > 1.5
...
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 60), title = 'CHOW versus WD2', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
ggsave('../figures/volcano/0vs2.pdf', height = 8, width = 6.5)

# 4 vs 0
res = lfcShrink(dds, contrast = c("condition", "4","0"), type = 'ashr')
annotation = data.frame("Ensembl.Gene.ID" = rownames(res), stringsAsFactors = FALSE) %>% left_join(., mapping, by = "Ensembl.Gene.ID")
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 40), title = 'CHOW versus WD4', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
ggsave('../figures/volcano/0vs4.pdf', height = 8, width = 6.5)

# 6 vs 0
res = lfcShrink(dds, contrast = c("condition", "6","0"), type = 'ashr')
annotation = data.frame("Ensembl.Gene.ID" = rownames(res), stringsAsFactors = FALSE) %>% left_join(., mapping, by = "Ensembl.Gene.ID")
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 55), title = 'CHOW versus WD6', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
ggsave('../figures/volcano/0vs6.pdf', height = 8, width = 6.5)

# 8 vs 0
res = lfcShrink(dds, contrast = c("condition", "8","0"), type = 'ashr')
annotation = data.frame("Ensembl.Gene.ID" = rownames(res), stringsAsFactors = FALSE) %>% left_join(., mapping, by = "Ensembl.Gene.ID")
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 55), title = 'CHOW versus WD8', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
ggsave('../figures/volcano/0vs8.pdf', height = 8, width = 6.5)

# 4 vs. 2
res = lfcShrink(dds, contrast = c("condition", "4","2"), type = 'ashr')
annotation = data.frame("Ensembl.Gene.ID" = rownames(res), stringsAsFactors = FALSE) %>% left_join(., mapping, by = "Ensembl.Gene.ID")
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 7.5), title = 'WD2 versus WD4', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
ggsave('../figures/volcano/2vs4.pdf', height = 8, width = 6.5)

# 6 vs. 2
res = lfcShrink(dds, contrast = c("condition", "6","2"), type = 'ashr')
annotation = data.frame("Ensembl.Gene.ID" = rownames(res), stringsAsFactors = FALSE) %>% left_join(., mapping, by = "Ensembl.Gene.ID")
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 10), title = 'WD2 versus WD6', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
ggsave('../figures/volcano/2vs6.pdf', height = 8, width = 6.5)

# 8 vs. 2
res = lfcShrink(dds, contrast = c("condition", "8","2"), type = 'ashr')
annotation = data.frame("Ensembl.Gene.ID" = rownames(res), stringsAsFactors = FALSE) %>% left_join(., mapping, by = "Ensembl.Gene.ID")
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 12.5), title = 'WD2 versus WD8', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
ggsave('../figures/volcano/2vs8.pdf', height = 8, width = 6.5)

# 6 vs. 4
res = lfcShrink(dds, contrast = c("condition", "6","4"), type = 'ashr')
annotation = data.frame("Ensembl.Gene.ID" = rownames(res), stringsAsFactors = FALSE) %>% left_join(., mapping, by = "Ensembl.Gene.ID")
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 5), title = 'WD4 versus WD6', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
ggsave('../figures/volcano/4vs6.pdf', height = 8, width = 6.5)

# 8 vs. 4
res = lfcShrink(dds, contrast = c("condition", "8","4"), type = 'ashr')
annotation = data.frame("Ensembl.Gene.ID" = rownames(res), stringsAsFactors = FALSE) %>% left_join(., mapping, by = "Ensembl.Gene.ID")
EnhancedVolcano(res, lab = annotation$Associated.Gene.Name, x = 'log2FoldChange', y = 'padj', xlim = c(-3, 3), ylim = c(0, 5), title = 'WD4 versus WD8', pCutoff = 0.05, FCcutoff = log2(1.5), colAlpha = 0.5, pointSize = 2.0)
...
### DE means padj < 0.05 and >1.5 fold-change either up or down
# =========================================
...
genes02 = as.data.frame(res02) %>% 
  tibble::rownames_to_column(var = "Ensembl.Gene.ID") %>% 
  left_join(., mapping, by = "Ensembl.Gene.ID") %>%
  mutate(FC_02 = 2 ** log2FoldChange,
         DE_02 = ifelse(padj < 0.05 & (FC_02 > 3/2 | FC_02 < 2/3) , "yes", "no")) %>% dplyr::select(Associated.Gene.Name, FC_02, DE_02)

res28 = lfcShrink(dds, contrast = c("condition", "8","2"), type = 'ashr')
genes28 = as.data.frame(res28) %>% 
  tibble::rownames_to_column(var = "Ensembl.Gene.ID") %>% 
  left_join(., mapping, by = "Ensembl.Gene.ID") %>%
  mutate(FC_28 = 2 ** log2FoldChange,
         DE_28 = ifelse(padj < 0.05 & (FC_28 > 3/2 | FC_28 < 2/3) , "yes", "no")) %>% dplyr::select(Associated.Gene.Name, FC_28, DE_28)

stopifnot(identical(genes02$Associated.Gene.Name, genes28$Associated.Gene.Name))

class_df = cbind(genes02, genes28 %>% dplyr::select(-Associated.Gene.Name)) %>% 
  mutate(class = ifelse(DE_02 == 'yes' & DE_28 == 'no', 'uniq_02',
                 ifelse(DE_02 == 'no' & DE_28 == 'yes', 'uniq_28',
                 ifelse(DE_02 == 'yes' & DE_28 == 'yes', 'both', 'none'))))

nrow(filter(class_df, class == 'uniq_02')) # 485
nrow(filter(class_df, class == 'uniq_28')) # 371
nrow(filter(class_df, class == 'both')) # 60
nrow(filter(class_df, class == 'none')) # 21820
```

```{r}
### Classify into 4 different patterns
### DE means padj < 0.05 and |LFC| > 0.5 
# =========================================
# 1: not DE between 02, not DE between 08
# 2: DE between 02, not DE between 08
# 3: DE between 02, DE between 08
# 4: not DE between 02, DE between 08
# separate by whether genes go up or done in each category
...
genes02 = as.data.frame(res02) %>% 
  ...
  left_join(., mapping, by = "Ensembl.Gene.ID") %>%
  mutate(
    log2FC_02 = log2FoldChange,
    DE_02 = ifelse(padj < 0.05 & abs(log2FoldChange) > 0.5, "yes", "no"),
    direct_02 = ifelse(log2FoldChange > 0, "up", "down")
  ) %>%
  dplyr::select(Associated.Gene.Name, log2FC_02, direct_02, DE_02)
...
genes28 = as.data.frame(res28) %>% 
  ...
  left_join(., mapping, by = "Ensembl.Gene.ID") %>%
  mutate(
    log2FC_28 = log2FoldChange,
    DE_28 = ifelse(padj < 0.05 & abs(log2FoldChange) > 0.5, "yes", "no"),
    direct_28 = ifelse(log2FoldChange > 0, "up", "down")
  ) %>%
  dplyr::select(Associated.Gene.Name, log2FC_28, direct_28, DE_28)
...
print(nrow(filter(class_df, class == 'uniq_02')))
print(nrow(filter(class_df, class == 'uniq_28')))
print(nrow(filter(class_df, class == 'both'))) 
print(nrow(filter(class_df, class == 'none'))) 
```
...
up_02_list = (
  class_df %>% 
    filter(class == 'uniq_02', direct_02 == 'up') %>% 
...
down_02_list = (
  class_df %>% 
    filter(class == 'uniq_02', direct_02 == 'down') %>% 
    dplyr::select(Associated.Gene.Name)
)
...
up_28_list = (
  class_df %>% 
    filter(class == 'uniq_28', direct_28 == 'up') %>% 
    dplyr::select(Associated.Gene.Name)
)
...
down_28_list = (
  class_df %>% 
    filter(class == 'uniq_28', direct_28 == 'down') %>% 
    dplyr::select(Associated.Gene.Name)
)
...
up02_up28_list = (
  class_df %>% 
    filter(class == 'both', direct_02 == 'up', direct_28 == 'up') %>% 
    dplyr::select(Associated.Gene.Name)
)
...
up02_down28_list = (
  class_df %>% 
    filter(class == 'both', direct_02 == 'up', direct_28 == 'down') %>% 
    dplyr::select(Associated.Gene.Name)
)
...
down02_up28_list = (
  class_df %>% 
    filter(class == 'both', direct_02 == 'down', direct_28 == 'up') %>% 
    dplyr::select(Associated.Gene.Name)
)
...
down02_down28_list = (
  class_df %>% 
    filter(class == 'both', direct_02 == 'down', direct_28 == 'down') %>% 
    dplyr::select(Associated.Gene.Name)
)
...
```{r}
# we input the above gene lists into KEGG pathway enrichment analysis via http://bioinformatics.sdstate.edu/go/ using their default background option: 
# all protein coding genes in the genome.

# we then filter the outputted KEGG pathways by their:
# 1. FDR < 0.05
# 2. any obvious irrelevant pathways (i.e., cancer, viral infection, etc.)
# 3. any super broad categories (i.e., meetabolic pathways)
...
plot_kegg(kegg_02_up, 'kegg_02_up', height=4, width=8, color='#9F2B68')
plot_kegg(kegg_02_down, 'kegg_02_down', height=4, width=8, color='#F675A8')
plot_kegg(kegg_28_up, 'kegg_28_up', height=5, width=8, color='#7BA472')
plot_kegg(kegg_28_down, 'kegg_28_down', height=2, width=8, color='#3B4463')
plot_kegg(kegg_02down_28up, 'kegg_02down_28up', height=3, width=8, color='#FFA500')
```
...
write.table(colnames(both %>% dplyr::select(-c(ID, Treatment))),
            '../processed_data/degenes/02_and_28_list_fc1.5.csv',
            quote = F, col.names = F, row.names = F)
...
write.table(colnames(uniq_02 %>% dplyr::select(-c(ID, Treatment))),
            '../processed_data/degenes/02_list_fc1.5.csv',
            quote = F, col.names = F, row.names = F)

uniq_28 = all_genes %>% dplyr::select(ID, Treatment, filter(class_df, class == 'uniq_28')$Associated.Gene.Name)
write.table(colnames(uniq_28 %>% dplyr::select(-c(ID, Treatment))),
            '../processed_data/degenes/28_list_fc1.5.csv',
            quote = F, col.names = F, row.names = F)
```

Gene lists were submitted to http://bioinformatics.sdstate.edu/go/ for GO and KEGG enrichment.

```{r}
### plot top KEGG terms for 02, 28, and both.

kegg_02 = read.csv('../processed_data/degenes/02_kegg_fc1.5_enrichment.csv', header = TRUE) %>% 
  filter(Fold.Enrichment > 3.5)
kegg_28 = read.csv('../processed_data/degenes/28_kegg_fc1.5_enrichment.csv', header = TRUE) %>% 
  filter(Fold.Enrichment > 3.5)
kegg_both = read.csv('../processed_data/degenes/02_28_kegg_fc1.5_enrichment.csv', header = TRUE)

kegg_both %>%
  ggplot(aes(x = reorder(Pathway, Fold.Enrichment), y = Fold.Enrichment, size = -log10(Enrichment.FDR))) +
  geom_point(alpha=0.5, shape=21, color="black", fill = '#6082B6') +
  scale_size(range = c(5, 20), name="-log10(Enrichment FDR)") +
  coord_flip() +
  labs(y = 'Fold Enrichment', title = 'DE between 0-2 week WD and 2-8 week WD') +
  theme(axis.title.y=element_blank())
ggsave('../figures/rnaseq_kegg_fc1.5_02_and_28.pdf', height = 4, width = 8)

# manually filter out unrelated viruses / cancer pathways.
kegg_02 %>%
  filter(!Pathway %in% c('Coronavirus disease', 'Measles', 'Epstein-Barr virus infection', 'Influenza A',
                         'Acute myeloid leukemia', 'Staphylococcus aureus infection', 
                         'Osteoclast differentiation')) %>%
  ggplot(aes(x = reorder(Pathway, Fold.Enrichment), y = Fold.Enrichment, size = -log10(Enrichment.FDR))) +
  geom_point(alpha=0.9, shape=21, color="black", fill = '#00A36C') +
  scale_size(range = c(3, 8), name="-log10(Enrichment FDR)") +
  coord_flip() +
  labs(y = 'Fold Enrichment', title = 'DE between 0-2 week WD') +
  theme(axis.title.y=element_blank(),
        panel.grid.major.y = element_line(colour = "grey80", linetype = 'dashed')
  ) 
ggsave('../figures/rnaseq_kegg_fc1.5_02.pdf', height = 5, width = 10)

# manually filter out unrelated viruses / cancer pathways.
kegg_28 %>%
  filter(!Pathway %in% c('Amoebiasis', 'Chemical carcinogenesis', 'Malaria', 'Platinum drug resistance', 
                         'Drug metabolism')) %>%
  ggplot(aes(x = reorder(Pathway, Fold.Enrichment), y = Fold.Enrichment, size = -log10(Enrichment.FDR))) +
  geom_point(alpha=0.9, shape=21, color="black", fill = '#DC143C') +
  scale_size(range = c(3, 8), name="-log10(Enrichment FDR)") +
  coord_flip() +
  labs(y = 'Fold Enrichment', title = 'DE between 2-8 week WD') +
  theme(axis.title.y=element_blank(),
        panel.grid.major.y = element_line(colour = "grey80", linetype = 'dashed')
  ) 
ggsave('../figures/rnaseq_kegg_fc1.5_28.pdf', height = 5, width = 11.5)
```
