...
go_02 = read.csv('../processed_data/degenes/02_go_enrichment.csv', header = TRUE) %>% 
  filter(Pathway.Genes < 500)

go_28 = read.csv('../processed_data/degenes/28_go_enrichment.csv', header = TRUE) %>%
  filter(Pathway.Genes < 500)

go_both = read.csv('../processed_data/degenes/02_and_28_go_enrichment.csv', header = TRUE) %>%
  filter(Pathway.Genes < 500)

go_both %>%
  ggplot(aes(x = reorder(Pathway, -Enrichment.FDR), y = -log10(Enrichment.FDR))) +
  geom_bar(stat="identity", fill = '#6082B6') + 
  coord_flip() +
  labs(y = '-log10(Enrichment FDR)', title = 'DE between 0-2 week WD and 2-8 week WD') +
  theme(axis.title.y=element_blank())
ggsave('../figures/rnaseq_GO_02_and_28.pdf', height = 6, width = 10)

go_02 %>%
  ggplot(aes(x = reorder(Pathway, -Enrichment.FDR), y = -log10(Enrichment.FDR))) +
  geom_bar(stat="identity", fill = '#00A36C') + 
  coord_flip() +
  labs(y = '-log10(Enrichment FDR)', title = 'DE between 0-2 week WD') +
  theme(axis.title.y=element_blank())
ggsave('../figures/rnaseq_GO_02.pdf', height = 6, width = 10)

go_28 %>%
  ggplot(aes(x = reorder(Pathway, -Enrichment.FDR), y = -log10(Enrichment.FDR))) +
  geom_bar(stat="identity", fill = '#DC143C') + 
  coord_flip() +
  labs(y = '-log10(Enrichment FDR)', title = 'DE between 2-8 week WD') +
  theme(axis.title.y=element_blank())
ggsave('../figures/rnaseq_GO_28.pdf', height = 6, width = 10)
```
