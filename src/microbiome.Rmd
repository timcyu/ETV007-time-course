...
firmicutes = data %>% 
  select(ID, Treatment, (tax_info %>% filter(Rank2 == 'p__Firmicutes'))$OTU)
...
ggsave('../figures/firmicutes_x_ba_totalpool.pdf', height = 4, width = 6)
```

```{r}
# correlation between Bacteroidetes and total BA species
bacteroidetes = data %>% 
  select(ID, Treatment, (tax_info %>% filter(Rank2 == 'p__Bacteroidetes'))$OTU)
...
ggsave('../figures/bacteroidetes_x_ba_totalpool.pdf', height = 4, width = 6)
```
```{r}
# correlation between Verrucomicrobia and total BA species
verruco = data %>% 
  select(ID, Treatment, (tax_info %>% filter(Rank2 == 'p__Verrucomicrobia'))$OTU)
...
ggsave('../figures/verruco_x_ba_totalpool.pdf', height = 4, width = 6)
```

Get number of significant correlations between each phyla and each BA (separated by tissue)
```{r}
biliary_BA_x_micro = read.csv('../processed_data/correlations/biliary_ba_x_microbiome.csv', 
                              header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>%
  left_join(., (tax_info %>% select(OTU, Rank2)), by=c("Microbiome_otu"="OTU"))

liver_BA_x_micro = read.csv('../processed_data/correlations/liver_ba_x_microbiome.csv', 
                            header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>%
  left_join(., (tax_info %>% select(OTU, Rank2)), by=c("Microbiome_otu"="OTU"))

intestinal_BA_x_micro = read.csv('../processed_data/correlations/intestinal_ba_x_microbiome.csv', 
                                 header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>%
  left_join(., (tax_info %>% select(OTU, Rank2)), by=c("Microbiome_otu"="OTU"))

total_BA_x_micro = read.csv('../processed_data/correlations/total_ba_x_microbiome.csv', 
                            header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>%
  left_join(., (tax_info %>% select(OTU, Rank2)), by=c("Microbiome_otu"="OTU"))
```
...
ggplot(data=NULL,
  ...
  geom_point(size=2) + 
  annotate("text", x=5000, y=20, size=4, label="italic(bicor) == 0.77", parse=TRUE) +
  labs(y='T-b-MCA (nmol)', x='bacteriodetes species 4457872 (rarefied counts)')
ggsave('../figures/bacteroidete_species_x_TbMCA_pos.pdf', height = 4.3, width = 5)
```
...
ggplot(data=NULL,
  ...
  geom_point(size=2) + 
  annotate("text", x=4000, y=20, size=4, label="italic(bicor) == -0.63", parse=TRUE) +
  labs(y='T-b-MCA (nmol)', x='bacteriodetes species 183446 (rarefied counts)')
ggsave('../figures/bacteroidete_species_x_TbMCA_neg.pdf', height = 4.3, width = 5)
```