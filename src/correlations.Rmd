...
```{r}
# merging rna-seq and lipid data for corr scatter plot, Gino
rnaseq_mapping_list = read.table('../raw_data/rnaseq_mapping_list.txt', header = TRUE, stringsAsFactors = FALSE)

all_genes["sampleID"] = rnaseq_mapping_list[match(rownames(all_genes), rnaseq_mapping_list$RNA_seq_ID), 2, drop=F]
liverlipids["sampleID"] = rownames(liverlipids)

sctplotdata <- merge(all_genes, liverlipids, by="sampleID")

write.csv(sctplotdata, 'nraseq_liverlipid_merge.csv')
#plot_data %>% ggplot(aes(x = gene_of_interest, y = lipid_of_interest)) + geom_point()
```
